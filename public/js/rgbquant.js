(function () {
    ...
    RgbQuant.prototype.dither = function (img, kernel, serpentine) {
        ...
        for (var y = 0; y < height; y++) {
            ...
            for (
                ...
                for (
                    var i = dir == 1 ? 0 : ds.length - 1,
                        end = dir == 1 ? ds.length : 0;
                    i !== end;
                    i += dir
                ) {
                    var x1 = ds[i][1] * dir,
    ...
    RgbQuant.prototype.colorStats2D = function colorStats2D(buf32, width) {
        ...
        boxes.forEach(function (box) {
            var effc = Math.max(
                    Math.round((box.w * box.h) / area) * self.boxPxls,
                    2
                ),
                histL = {},
                col;
    ...
    RgbQuant.prototype.sortPal = function sortPal() {
        ...
        this.idxi32.sort(function (a, b) {
            ...
            var lumDiff =
                lumGroup(+hslB.l.toFixed(2)) - lumGroup(+hslA.l.toFixed(2));
            if (lumDiff) return -lumDiff;

            var satDiff =
                satGroup(+hslB.s.toFixed(2)) - satGroup(+hslA.s.toFixed(2));
    ...
    HueStats.prototype.check = function checkHue(i32) {
        ...
        var r = i32 & 0xff,
            ...
            hg =
                r == g && g == b
                    ? -1
                    : hueGroup(rgb2hsl(r, g, b).h, this.numGroups),
            gr = this.stats[hg],
            min = this.minCols;
