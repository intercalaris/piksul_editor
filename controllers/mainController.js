...
module.exports = {
  ...
  getEditor: (req, res) => {
    res.render('editor.ejs', { 
      project: { id: '', original_image: '', block_size: '', palette_size: '' } 
    });
  },
  ...
  createOrUpdateProject: async (req, res) => {
    ...
    try {
      if (project_id) {
        ...
        await ProjectModel.updateProject(project_id, originalImagePath, editedImagePath, block_size, palette_size, tolerance);
        res.json({ project_id, message: "Project updated successfully" });
      } else {
        // Insert a new project
        const newProjectId = await ProjectModel.insertProject(block_size, palette_size, tolerance, userId);
        const { originalImagePath, editedImagePath } = await ProjectModel.saveImages(newProjectId, originalImage, editedImage);
        await ProjectModel.updateProject(newProjectId, originalImagePath, editedImagePath, block_size, palette_size, tolerance);
